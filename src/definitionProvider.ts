...
export class HoloDevDefinitionProvider implements vscode.DefinitionProvider {
	...
	async provideDefinition(
		...
		const currentModule = currentModuleName
			? this.index.getModule(currentModuleName)
			: undefined;
		...
		switch (ctx.kind) {
			case "variable": {
				...
				if (prop) {
					...
					return new vscode.Location(
						currentModule.uri,
						new vscode.Position(currentModule.defmoduleLine, 0),
					);
				}
			...
			case "component": {
				...
				const mod =
					...
					this.index.getModuleByShortName(ctx.name);
				...
				if (target === "template" && mod.templateLine !== undefined) {
					return new vscode.Location(
						mod.uri,
						new vscode.Position(mod.templateLine - 1, 0),
					);
				}
				if (target === "init" && mod.initLine !== undefined) {
					return new vscode.Location(
						mod.uri,
						new vscode.Position(mod.initLine - 1, 0),
					);
				}
				...
				return new vscode.Location(
					mod.uri,
					new vscode.Position(mod.defmoduleLine - 1, 0),
				);
			}
			...
			case "field_access": {
				...
				const resolvedFullName =
					resolveComponentName(prop.type, document) ?? prop.type;
				const resource = this.index.getResource(resolvedFullName);

				if (resource) {
					const attr = resource.attributes.find(
						(a) => a.name === ctx.fieldName,
					);
					if (attr && attr.line > 0) {
						...
						if (uri) {
							return new vscode.Location(
								uri,
								new vscode.Position(attr.line - 1, 0),
							);
						}
					}

					const rel = resource.relationships.find(
						(r) => r.name === ctx.fieldName,
					);
					if (rel && rel.line > 0) {
						const uri = this.index.getModule(resolvedFullName)?.uri;
						if (uri) {
							return new vscode.Location(
								uri,
								new vscode.Position(rel.line - 1, 0),
							);
						}
					}
				}
