...
export class FieldCodeActionProvider implements vscode.CodeActionProvider {
  ...
  private fixUnknownField(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
    const actions: vscode.CodeAction[] = [];
    ...
    const sorted = [...suggested, ...fields.filter(f => !suggested.includes(f))];

    for (const field of sorted) {
      const isSuggested = suggested.includes(field);
      const label = isSuggested ? `Did you mean "${field}"?` : `Replace with "${field}"`;
  ...
  private fixUnknownProp(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
    const actions: vscode.CodeAction[] = [];
    ...
    const sorted = [...suggested, ...props.filter(p => !suggested.includes(p))];

    for (const prop of sorted) {
      const isSuggested = suggested.includes(prop);
      const label = isSuggested ? `Did you mean "${prop}"?` : `Replace with "${prop}"`;
      const action = new vscode.CodeAction(label, vscode.CodeActionKind.QuickFix);
      action.edit = new vscode.WorkspaceEdit();
  ...
  private fixMissingProp(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
    const actions: vscode.CodeAction[] = [];

    // Extract prop name and type from message: Missing required prop "to" (module) on <Link>.
    const match = diagnostic.message.match(/Missing required prop "(\w+)" \(([^)]+)\)/);
